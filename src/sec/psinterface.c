    ...
    if (dbData && dbSize)
    {
        ...
        // this local scoping intended, for destroying large cbor instances after use
        {
            ...
            if (PS_DATABASE_SECURITY == database)
            {
                ...
                /*     VERIFY_CBOR_SUCCESS_OR_OUT_OF_MEMORY(TAG, cborFindResult, "Failed Finding PCONF Name Value."); */
                /* } */
        ...
        // this local scoping intended, for destroying large cbor instances after use
        {
		  ...
		  + credCborLen + /* pconfCborLen + */ resetPfCborLen + crlCborLen
            ...
            if (PS_DATABASE_SECURITY == database)
            {
                ...
                /*     VERIFY_CBOR_SUCCESS_OR_OUT_OF_MEMORY(TAG, cborEncoderResult, "Failed Adding Pconf Name."); */
                /*     cborEncoderResult |= cbor_encode_byte_string(&resource, pconfCbor, pconfCborLen); */
                /*     VERIFY_CBOR_SUCCESS_OR_OUT_OF_MEMORY(TAG, cborEncoderResult, "Failed Adding Pconf Value."); */
                /* } */
