...
//-------------------------------------------------------------------------------------------------
/* int RBRequestTokenCmp(OCServerRequest *target, OCServerRequest *treeNode) */
/* { */
...
/*         if (tokenLength) */
/*         { */
/*             serverRequest->requestToken = (uint8_t*) OICMalloc(tokenLength); */
 ...
 *     OCStackResult
 */
/* once the handler has created a response, this routine prepares the
   outbound CAResponseInfo_t pkg, cbor encodes payload, and sends it */
/* FIXME: eliminate CAResponseInfo_t and use OCEntityHandlerResponse all the way through? */
OCStackResult HandleSingleResponse(OCEntityHandlerResponse * ehResponse)
{
    ...
    responseInfo.info.messageId = serverRequest->info.messageId;
    responseInfo.info.resourceUri = getPathFromRequestURL(serverRequest->info.resourceUri);
    responseInfo.result = ConvertEHResultToCAResult(ehResponse->ehResult, serverRequest->method);
    OIC_LOG_V(INFO, TAG, "%s result afer eh conversion: 0x%04x", __func__, responseInfo.result);

    responseInfo.info.dataType = CA_RESPONSE_DATA;

    if(serverRequest->notificationFlag && serverRequest->info.type == CA_MSG_CONFIRM) // OC_HIGH_QOS)
    {
        responseInfo.info.type = CA_MSG_CONFIRM;
    }
    else if(serverRequest->notificationFlag && serverRequest->info.type != CA_MSG_CONFIRM) // OC_HIGH_QOS)
    {
        responseInfo.info.type = CA_MSG_NONCONFIRM;
    }
    else if(!serverRequest->notificationFlag && !serverRequest->slowFlag &&
            serverRequest->info.type == CA_MSG_CONFIRM) // OC_HIGH_QOS)
    {
    ...
    memcpy(responseInfo.info.token, serverRequest->info.token, serverRequest->info.tokenLength);
    responseInfo.info.tokenLength = serverRequest->info.tokenLength;
    ...
    if (ehResponse->payload)
    {
        ...
        {
            if (OCF_OPTION_CONTENT_FORMAT_VERSION == ehResponse->sendVendorSpecificHeaderOptions[i].optionID)
            {
                payloadVersion =
    ...
    if (responseInfo.info.numOptions > 0)
    {
        ...
        if(serverRequest->observeResult == OC_STACK_OK
           && serverRequest->observationOption != MAX_SEQUENCE_NUMBER + 1)
        {
            // FIXME: where does it say OBSERVE must be first option?
        ...
        if (ehResponse->payload)
        {
            ...
            {
                //optionsPointer->protocolID = CA_COAP_ID;
                optionsPointer->optionID = OCF_OPTION_CONTENT_FORMAT_VERSION;
            ...
            case OC_FORMAT_VND_OCF_CBOR:
                        ...
                        responseInfo.info.payloadSize > 0)
                {
                    responseInfo.info.payloadFormat = OCToCAPayloadFormat(
                            serverRequest->info.acceptFormat);
                    if (CA_FORMAT_UNDEFINED == responseInfo.info.payloadFormat)
                    {
                        responseInfo.info.payloadFormat = CA_FORMAT_APPLICATION_CBOR;
                    }
                    if ((CA_FORMAT_APPLICATION_VND_OCF_CBOR == serverRequest->info.acceptFormat))
                    {
                        // Add versioning information for this format
                        responseInfo.info.payloadVersion = serverRequest->info.acceptVersion;
...
/* OCStackResult XHandleSingleResponse(OCEntityHandlerResponse * ehResponse) */
/* { */
/*     OCStackResult result = OC_STACK_ERROR; */
/*     CAEndpoint_t responseEndpoint = {.adapter = CA_DEFAULT_ADAPTER}; */
/*     CAResponseInfo_t responseInfo = {.result = CA_EMPTY}; */
...
OCStackResult HandleAggregateResponse(OCEntityHandlerResponse * ehResponse)
{
    ...
    if(request)
    {
        if(!serverResponse)
        {
            ...
            _oocf_request_cache_put(request);
...
/* OCStackResult XHandleAggregateResponse(OCEntityHandlerResponse * ehResponse) */
/* { */
/*     if(!ehResponse || !ehResponse->payload) */
/*     { */
/*         OIC_LOG(ERROR, TAG, "HandleAggregateResponse invalid parameters"); */
