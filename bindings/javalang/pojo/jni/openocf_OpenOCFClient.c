 ...
 * Class:     openocf_OpenOCFClient
 * Method:    _coExhibit
 * Signature: (ILjava/lang/String;Lopenocf/Endpoint;ILopenocf/app/CoResourceSP;Ljava/util/List;)V
 */
JNIEXPORT void JNICALL
				       ...
				       jobject j_coap_options)
{
    ...
    OIC_LOG_V(DEBUG, TAG, "%s method: %u", __func__, j_method);

    const char *c_uri = (char*) (*env)->GetStringUTFChars(env, j_uri, NULL);
    OIC_LOG_V(DEBUG, TAG, "%s uri: %s", __func__, c_uri);
    ...
    OCStackResult rc = OCDoRequest(&c_TxnId,	/* OCDoHandle = void* */
    ...
    OIC_LOG_V(DEBUG, TAG, "%s OCDoRequest executed, rc: %u", __func__, rc);
    if (rc != OC_STACK_OK) {
        OIC_LOG_V(DEBUG, TAG, "%s throwing stack exception on rc: %u", __func__, rc);
        char msg[24];
        switch (rc) {
        case OC_STACK_INVALID_METHOD:
            snprintf(msg, 24, "%u", (int)j_method);
            break;
        default:
            snprintf(msg, 24, "foo");
        }
        throw_stack_exception(env, __func__, __FILE__, __LINE__, rc, msg);
    }
