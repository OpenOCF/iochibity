...
headers. First build makeheaders, then use it:

[source,]
----
$ bazel build tools/makeheaders -f bazel-bin/mkhdrs.dat
$ bazel build tools/makeheaders -f bazel-bin/mkhdrs.dat -H > include/openocf_h.h
----
