...
OCStackApplicationResult react_to_get(void* ctx, OCDoHandle handle,
                                      oocf_inbound_response * oocf_inbound_response)
{
    if (oocf_inbound_response == NULL)
    {
        clog_info(CLOG(MY_LOGGER), "react_to_get received NULL oocf_inbound_response");
    ...
    clog_info(CLOG(MY_LOGGER), "StackResult: %d", oocf_inbound_response->result);
    clog_info(CLOG(MY_LOGGER), "SEQUENCE NUMBER: %d", oocf_inbound_response->sequenceNumber);
    //OIC_LOG_PAYLOAD(INFO, oocf_inbound_response->payload);
    clog_info(CLOG(MY_LOGGER), ("=============> Get Response"));

    if (oocf_inbound_response->numRcvdVendorSpecificHeaderOptions > 0)
    {
        OIC_LOG (INFO, TAG, "Received vendor specific options");
        uint8_t i = 0;
        struct oocf_coap_options *rcvdOptions = oocf_inbound_response->rcvdVendorSpecificHeaderOptions;
        for( i = 0; i < oocf_inbound_response->numRcvdVendorSpecificHeaderOptions; i++)
        {
            if (((CAHeaderOption_t)rcvdOptions[i]).protocolID == CA_COAP_ID)
            {
                clog_info(CLOG(MY_LOGGER), "Received option with OC_COAP_ID and ID %u with",
                        ((CAHeaderOption_t)rcvdOptions[i]).optionID );
               if (OCF_CONTENT_FORMAT_VERSION == ((CAHeaderOption_t)rcvdOptions[i]).optionID) /* 2053 */
               {
                    uint16_t versionValue = rcvdOptions[i].optionData[0] * 256
                            + rcvdOptions[i].optionData[1];
                    clog_info(CLOG(MY_LOGGER), "Received version value of %u", versionValue);
               }
               if (COAP_OPTION_CONTENT_FORMAT == ((CAHeaderOption_t)rcvdOptions[i]).optionID) /* 12 */
               {
                    uint16_t formatValue = rcvdOptions[i].optionData[0] * 256
                            + rcvdOptions[i].optionData[1];
                   clog_info(CLOG(MY_LOGGER), "Received format value of %u", formatValue);
               }
                OIC_LOG_BUFFER(INFO, TAG, ((CAHeaderOption_t)rcvdOptions[i]).optionData,
...
int get_doxm(OCDiscoveryPayload* discovery_payload, CATransportAdapter_t transport)
{
    ...
    if (found) {
        ...
        clog_info(CLOG(MY_LOGGER), "%s%s", OCCreateEndpointString(eps), doxm_path);
        clog_info(CLOG(MY_LOGGER), "dev flags is 0x%04x", dev.flags);
        clog_info(CLOG(MY_LOGGER), "dev adapter is %d", dev.adapter);
        ...
        cbData.cb = react_to_get;
        cbData.context = (void*)&context;
        cbData.cd = NULL;

        OCMethod method = OC_REST_GET;

        OCStackResult ret = OCDoRequest(&handle, method, doxm_path,
                                        &dev,
                                        NULL, /* payload */
                                        CT_ADAPTER_IP,
                                        OC_LOW_QOS,
...
void resource_discovery_cb(void* ctx,
                           OCDoHandle h,
    ...
    get_doxm(inbound_response->payload, OOCF_UDP);
